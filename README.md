...
Can PromptShield's methodology be successfully replicated using alternative model architectures?
What performance can be achieved at deployment-critical low FPR thresholds (0.05% - 1%)?
How does temperature scaling affect probability calibration and deployment viability?
What are the practical considerations for production deployment of such systems?
...
Results
Low FPR Performance
FPR LevelTPR (This Work)TPR (PromptShield)Required Threshold0.05%35.8%~45-50%*0.9920.10%43.3%~55-60%*0.9880.50%58.4%~65%*0.9661.00%66.3%73.82%0.947
...
Model Architecture Differences: Llama 3.1 8B (decoder-only, autoregressive) vs. FLAN-T5-large (encoder-decoder)
Quantization: 4-bit QLoRA vs. likely full-precision training
Training Details: Different hyperparameter choices, number of epochs, optimizer settings
Dataset Sampling: Possible variations in random seed and train/test splits
...
Low FPR evaluation at 0.05%, 0.1%, 0.5%, and 1% FPR
ROC curve visualization with focus on low FPR region
Three-tier adversarial testing framework
...
Dependencies
bashpip install transformers datasets accelerate bitsandbytes scipy scikit-learn peft trl
Running the Notebook

Obtain HuggingFace access token for Llama 3.1 models
Open the training notebook in Google Colab or Jupyter
Configure HuggingFace authentication
Execute cells sequentially
Results will be saved to Google Drive (Colab) or local directory
...
Language Coverage: Primarily trained and evaluated on English text
Attack Evolution: Model trained on known attack patterns; new attack types may evade detection
Adversarial Robustness: Not explicitly trained against adaptive adversaries
False Negative Rate: 56.7% of attacks missed at 0.1% FPR deployment point
Computational Cost: 10-20ms inference latency per request may be prohibitive for high-throughput systems

Future Directions

Multi-Model Comparison: Systematic evaluation of different model sizes and architectures
Ablation Studies: Quantifying impact of individual components (temperature scaling, LoRA rank, quantization)
Extended Evaluation: Testing on additional datasets and real-world attack samples
Adversarial Training: Incorporating adversarial examples during training
Model Distillation: Creating smaller, faster models for edge deployment
Multilingual Evaluation: Extending to non-English languages
Ensemble Methods: Combining multiple models for improved robustness

Key Contributions

Independent validation of PromptShield's low FPR evaluation methodology
Demonstration that the approach generalizes to alternative model architectures (Llama vs. FLAN-T5)
Deployment-focused analysis with practical threshold recommendations
Open-source implementation enabling further research
Comprehensive documentation of replication process and results

Citations
If you use this work, please cite:
bibtex@misc{promptshield_replication_2025,
...
bibtex@inproceedings{jacob2025promptshield,
...
Touvron, H., et al. (2023). Llama 2: Open Foundation and Fine-Tuned Chat Models. arXiv preprint arXiv:2307.09288. Claude 4.5 Sonnet.
